	...
	$(function(){
			...
			$.ajax({
				...
				success:function (data) {
					if(data.success){
						...
						// pageList(1,2);
						pageList(1,//操作后停留在当前页（参数无需改动，插件自带）
								 $("#activityPage").bs_pagination('getOption','rowsPerPage'));//操作后维持已经设置好的每页展示的记录数（参数无需改动，插件自带）
					}else{
						alert("添加市场活动失败!");
					}
				}
		...
		$("#editBtn").click(function () {
			...
			if($xz.length==0){
				...
				$.ajax({
					...
					success:function (data) {
						var html = "<option>--请选择--</option>";
						$.each(data.uList,function (i,user) {
		...
		$("#updateBtn").click(function () {
			$.ajax({
				...
				success:function (data) {
					if(data.success){
						pageList($("#activityPage").bs_pagination('getOption','currentPage'),//操作后停留在当前页（参数无需改动，插件自带）
		...
		$.ajax({
			...
			success:function (data) {
				...
				$.each(data.dataList,function (i,activity) {
					...
					html+= '<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'activity/detail.do?id='+activity.id+'\';">'+ activity.name +'</a></td>';
